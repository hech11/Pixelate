...
void main() {
	...
	static const char* DefaultFilename = "NewShader.pxShader";
	...
	void ShaderGenerator::GenerateToDisk(std::filesystem::path path)
	{
		...
		while (true)
		{
			...
			path = "assets" / path / std::string(name.str() + extra.str() + ".pxShader");
			file.close();
			file.open(path);
			name << extra.str();
		...
		file << buffer;
		file.close();
		AssetManager::ImportAsset(path.string());
		Renderer2D::GetShaderLibrary().Add(name.str(), path.string());
	}
